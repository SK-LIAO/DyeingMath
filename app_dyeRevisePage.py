...
class dyeRevisePage(GUI):
    def __init__(self, parent, controller):
        ...
        lb1 = tk.Label(frame1,text='濃度1',fg='#F00')
        lb2 = tk.Label(frame1,text='濃度2',fg='#F00')
        lb3 = tk.Label(frame1,text='濃度3',fg='#F00')
    ...
    def analysis(self):
        ...
        self.ax.plot(curve1[:,0],curve1[:,1],curve1[:,2])
        self.ax.plot(curve2[:,0],curve2[:,1],curve2[:,2])
        self.ax.scatter([0],[0],[0],s=120,c='#F00',marker='*')
        for x,y,z,label in zip(mat2[:,0],mat2[:,1],mat2[:,2],['Recipe 1','Recipe 2','Recipe 3','Recipe 4']):
            self.ax.text(x,y,z,label)
        self.ax.set_xlabel("DL ",c='#F00',fontsize=20)
        self.ax.set_ylabel("Da*",c='#F00',fontsize=20)
        self.ax.set_zlabel("Db*",c='#F00',fontsize=20)
